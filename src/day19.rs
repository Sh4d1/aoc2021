...
impl Point {
    ...
    fn orientation(&self, i: u8) -> Point {
        match i {
    ...
    fn rotate(&mut self, index: u8) {
        *self = self.orientation(index);
    }

    fn distance(&self, p: &Self) -> isize {
        (self.x - p.x).abs() + (self.y - p.y).abs() + (self.z - p.z).abs()
    }
}

fn search(scanners: &mut Vec<Scanner>, scanner: usize, visited: &mut Vec<bool>) {
    visited[scanner] = true;
    ...
    for i in 0..scanners.len() {
        ...
        let mut d: HashMap<Point, (u8, u32)> = HashMap::new();

        for o in &scanners[i].beacons {
            for orientation in 0..24 {
                let oriented = o.orientation(orientation);
                for b in &scanners[scanner].beacons {
                    d.entry(*b - oriented)
                        .and_modify(|f| f.1 += 1)
        ...
        for (pos, (o, n)) in d {
            ...
            for b in &mut scanners[i].beacons {
                b.rotate(o);
            }

            scanners[i].pos = scanners[scanner].pos + pos;
            if !visited[scanners[i].index] {
                search(scanners, scanners[i].index, visited)
            }
        }
    }
}
    ...
    input
        ...
        .map(|(index, l)| {
            let beacons = l
                ...
                .skip(1)
                .map(|l| {
                    let p: Vec<isize> = l.split(',').map(|d| d.parse().unwrap()).collect();
                    Point {
                        x: p[0],
                        y: p[1],
                        z: p[2],
...
pub fn part1(input: &[Scanner]) -> usize {
    ...
    search(&mut input, 0, &mut visited);
    input
        .iter()
        .flat_map(|s| s.beacons.clone().into_iter().map(move |b| b + s.pos))
        .collect::<HashSet<_>>()
        .len()
}

#[aoc(day19, part2)]
pub fn part2(input: &[Scanner]) -> usize {
    let mut visited = vec![false; input.len()];
    let mut input = input.to_vec();
    search(&mut input, 0, &mut visited);
    input
